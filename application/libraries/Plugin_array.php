...
class Plugin_Array{
     ...
     *     array('id' => 2, 'value' => '2-1'),
     * );
     * $hashmap = Plugin_Array::hashMap($rows, 'id', 'value');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // array(
     ...
     *     array('id' => 2, 'value' => '2-1'),
     * );
     * $hashmap = Plugin_Array::hashMap($rows, 'id');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // array(
     ...
     * );
     * $values = Plugin_Array::groupBy($rows, 'parent');
     *
     * dump($values);
     *   // 按照 parent 分组的输出结果为
     *   // array(
     ...
     * );
     *
     * $tree = Plugin_Array::tree($rows, 'id', 'parent', 'nodes');
     *
     * dump($tree);
     *   // 输出结果为：
     *   // array(
     ...
     * @code php
     * $refs = null;
     * $tree = Plugin_Array::tree($rows, 'id', 'parent', 'nodes', $refs);
     * 
     ...
     * );
     *
     * $rows = Plugin_Array::sortByCol($rows, 'id', SORT_DESC);
     * dump($rows);
     * // 输出结果为：
     * // array(
     * //   array('id' => 6, 'value' => '6-1', 'parent' => 3),
     ...
     * 用法：
     * @code php
     * $rows = Plugin_Array::sortByMultiCols($rows, array(
     *     'parent' => SORT_ASC, 
     *     'name' => SORT_DESC,
     * ));
     * @endcode
